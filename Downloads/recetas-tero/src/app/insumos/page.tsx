...
import { Plus, Pencil, Trash2, TrendingUp, TrendingDown, Minus, LineChart as LineChartIcon } from 'lucide-react'
...
export default function InsumosPage() {
  ...
  async function fetchInsumos() {
    ...
    // Obtener precios anteriores (el último antes del actual)
    ...
    const insumosCompletos: InsumoCompleto[] = (insumosData || []).map(insumo => {
      ...
      // Buscar precio anterior (el más reciente que no sea el actual)
  ...
  function handleOpenModal(insumo?: InsumoCompleto) {
    if (insumo) {
      ...
      setForm({
        ...
        precio: insumo.precio_actual?.toString() || '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editingId) {
      ...
      // Si hay precio y proveedor, actualizar precio
      if (precio > 0 && proveedorId) {
        ...
        if (precioError) {
          console.error('Error updating precio:', precioError)
        }
      }
      ...
      // Si hay precio y proveedor, insertar precio inicial
      if (precio > 0 && proveedorId && newInsumo) {
        await supabase
          .from('precios_insumo')
          .insert({
  ...
  const filteredInsumos = filtroCategoria
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600">Mercadería y materias primas</p>
        </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredInsumos.length === 0 ? (
                  ...
                  filteredInsumos.map((insumo) => {
                    ...
                    const costoConIva = insumo.precio_actual ? calcularCostoConIva(insumo.precio_actual, insumo.iva_porcentaje) : null
                    const costoFinal = insumo.precio_actual ? calcularCostoFinal(insumo.precio_actual, insumo.iva_porcentaje, insumo.merma_porcentaje) : null
                    ...
                    return (
                      <tr key={insumo.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-2 py-1.5 text-right text-gray-500">
                          {formatCurrency(insumo.precio_anterior)}
                        </td>
                        ...
                        <td className="px-2 py-1.5 text-right font-medium text-gray-900">
                          {formatCurrency(insumo.precio_actual)}
                        </td>
                        <td className="px-1 py-1.5 text-center">
                        ...
                        <td className="px-2 py-1.5 text-right text-gray-600">
                          {formatCurrencyDecimal(costoConIva)}
                        </td>
                        <td className="px-1 py-1.5 text-center">
                        ...
                        <td className="px-2 py-1.5 text-right font-bold text-green-700 bg-green-50">
                          {formatCurrencyDecimal(costoFinal)}
                        </td>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="border-t pt-4 mt-4">
            ...
            {form.precio && !form.proveedor_id && (
              <p className="text-xs text-amber-600 mt-1">Seleccioná un proveedor para guardar el precio</p>
            )}
          </div>
          ...
          {(form.precio && parseFloat(form.precio) > 0) && (
            <div className="bg-gray-50 rounded-lg p-4 space-y-2">
              ...
              {(() => {
                ...
                const costoConIva = calcularCostoConIva(precio, iva)
                const costoFinal = calcularCostoFinal(precio, iva, merma)

                return (
                  <div className="grid grid-cols-3 gap-4 text-sm">
                    <div>
                      <p className="text-gray-500">Precio</p>
                      <p className="font-medium">{formatCurrency(precio)}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      <p className="text-gray-500">+ Merma ({merma}%)</p>
      ...
      <Modal
        ...
        title={`Historial de Precios — ${historialNombre}`}
      >
        {historialLoading ? (
