...
import {
  COLORS, MESES, DIAS_SEMANA, VENDEDORES, TIPOS_EVENTO, MENUS,
  TIPOS_MENU, TURNOS, SALONES, COBRADORES, FERIADOS_ARGENTINA,
  CATEGORIAS_POR_MENU, PLATOS_POR_MENU, CONCEPTOS_INGRESO,
  CONCEPTOS_EGRESO, SOCIOS, CAJAS
} from './constants';

import {
  formatCurrency, formatMoney, formatNumber, formatNumberInput,
...
export default function App() {
  // Auth hook
  const {
    user, userRole, userTabsPermitidas, userVerPrecios, authLoading,
    loginForm, setLoginForm, loginError, loginLoading,
    showPassword, setShowPassword,
    showPasswordReset, setShowPasswordReset,
    newPassword, setNewPassword, newPassword2, setNewPassword2,
    passwordResetLoading, passwordResetError,
    canCreate, canEdit, canDelete,
    displayPrice, handleLogin, handlePasswordChange, handleLogout
  } = useAuth();

  // Estado de pagos (necesario para useEventos)
  const [pagos, setPagos] = useState([]);

  // Eventos hook
  const {
    eventos, setEventos, loading, showModal, setShowModal, saving, setSaving,
    editMode, setEditMode, eventoEdit, setEventoEdit, selectedEvento, setSelectedEvento,
    nuevoEvento, setNuevoEvento,
    searchTerm, setSearchTerm, filterVendedor, setFilterVendedor,
    filterMes, setFilterMes, filterYear, setFilterYear,
    filterEstado, setFilterEstado, sortConfig, setSortConfig,
    filterMesProximos, setFilterMesProximos, filterVendedorProximos, setFilterVendedorProximos,
    ...
    filteredEventos, stats, eventosPorMes, eventosPorVendedor,
    eventosPorTipo, eventosPorMenu, eventosPorSalon
  } = useEventos({ user, pagos, fetchAuditoriaEventos: null });
...
<body>
  ...
  const generarRecibo = (evento, pagoData, saldoAnterior) => {
    ...
    // Saldo pendiente + IVA
    // El total del evento está sin IVA, los pagos se descuentan directamente
